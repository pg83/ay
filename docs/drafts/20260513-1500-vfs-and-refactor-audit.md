...
4. `./yatool make -j 0 -G tools/archiver > .out/m2.json` →
   normalize → `our_sha256 == ref_sha256 == b4d440096b9c…`
5. `./yatool make -j 0 -G devtools/ymake/bin > .out/m3.json` →
...
8. `./yatool make -j 0 -G tools/archiver` даёт `c3078d56…`.
...
- Аллокации в `./yatool make -j 0 -G devtools/ymake/bin` (heap profile):
