...
// STDOUT_NOAUTO step routes its python output through the bare-token path: pb2_arg
// uses to_build_root(X.proto)==X.proto (not is_arc_src), so the PY_SRCS token is
// the bare `X__intpy3___pb2.py`. That bare token, with no `/`, makes uniq_suffix
// empty (no `.kqhj` yapyc3 suffix) and — carrying no `${ARCADIA_BUILD_ROOT}`
// literal — embeds via objcopy, NOT the rescompiler `_raw.auxcpp` path checked-in
// protos use. The PB and py3cc nodes also carry the generated `.proto` producer's
// transitive `$(S)` closure (its OUTPUT_INCLUDES protos).
...
`,
	...
	// (4) objcopy resfs node: hash over the bare tokens + PY3_PROTO tag.
...
// to _PROTOC_FLAGS. The python proto command renders `$_PROTOC_FLAGS` after
// `--python_out` and before the trailing source token — as the cpp side appends
// d.protocFlags after --cpp_out. A sibling PROTO_LIBRARY without the macro must NOT
// carry the flag.
...
`,
	...
	// The fatal-warnings module's py PB node (the .pyi shares it) must carry
	// --fatal_warnings, after --python_out and before the trailing source token.
...
// rescompiler `_raw.auxcpp` path with a path-id-suffixed yapyc3 — it must NOT be
// re-routed to objcopy by the generated-proto handling.
...
// The autoincluded linters.make.inc gates CLANG_WARNINGS(-Wimplicit-fallthrough)
// behind `IF (MODULE_LANG == CPP)`, evaluated per submodule of a PROTO_LIBRARY
// multimodule: the _CPP_PROTO submodule has MODULE_LANG==CPP, so its generated
// .pb.cc compile receives the warning; the _PY3_PROTO submodule (MODULE_LANG PY3)
// does not, so the optimized-py-proto resource aux C++ object must NOT carry it.
