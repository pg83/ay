...
Study BOTH sides before you touch anything: the upstream code (the mechanism you must reproduce) AND our own code (the mechanisms that already exist). Do NOT work around our existing machinery — reuse it, extend it, or route data through it. Example: if a file has already been parsed, never parse it again — thread the existing parse result to where it is needed. The same rule holds for everything: if a value is already computed, cached, resolved, or loaded somewhere, plumb it through rather than recomputing it or building a parallel path. A second parse, a duplicate cache, a shadow data structure that bypasses what we already have is a workaround and is wrong, even if it produces the right number. Find the right seam in the existing design and wire your change into it.
