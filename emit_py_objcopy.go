...
// generated resource (RUN_PROGRAM OUT/OUT_NOAUTO/STDOUT, COPY output, …) lives in
// the codegen registry keyed by its output VFS; copyFileOutputVFS canonicalizes the
// raw RESOURCE path (${BINDIR}/X, $(B)/<unit>/X, or a plain name) to that key. When
// found, the input is the $(B) build artifact and the producer ref is returned so the
// objcopy node depends on it. Otherwise the path is an ordinary source file: the
// fallback VFS (its $(S) location) is used with no extra dep.
func resolveResourceInput(ctx *GenCtx, instance ModuleInstance, rawPath string, fallback VFS) (VFS, NodeRef) {
	output := copyFileOutputVFS(instance.Path.rel(), rawPath)
