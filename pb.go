...
func emitProtoSrcs(ctx *genCtx, instance ModuleInstance, d *moduleData, peerContribs peerGlobalContribs) (NodeRef, string, bool) {
	...
	for _, co := range codegenOutputs {
		...
		// from the .cc.o's IncludeInputs. protoc / event2cpp emits the
		// .pb.cc and .pb.h together; REF treats the sibling as a
		// producer-only output, not a transitive input of the downstream
		// CC (both are co-generated by the same PB/EV producer node).
		siblingPbH := strings.TrimSuffix(co.pbCC, ".cc") + ".h"
		ccIn.IncludeInputs = filterSiblingPbH(walkClosure(ctx, instance, co.pbCC, moduleInputs), siblingPbH)
