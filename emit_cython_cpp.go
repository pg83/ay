...
func (e *EmitContext) emitCythonCppPlanned(plans []CythonStmtPlan) {
	...
	for i := range plans {
		...
		cyRef := p.cyRef
		sourceClosure := walkClosureTail(e.scanner, srcVFS, srcScanIn).flat()
		...
		if pxdVFS, ok := resolveCythonPxd(ctx, instance, stmt.Pxd); ok {
			pxdClosure := walkClosure(e.scanner, pxdVFS, srcScanIn).flat()
...
func (e *EmitContext) cythonCppInducedSets(src VFS, cMode bool, scanIn ScanContext) CythonCppInduced {
	...
	for _, v := range py3CythonOutputIncludes {
		...
		cl := walkClosureTail(scanner, v, scanIn).flat()
