...
func TestEmitLD_SplitDwarfCommandsCarryDistbuildEnv(t *testing.T) {
	...
	for _, idx := range []int{4, 5, 6} {
		...
		if got.Cmds[idx].Env[0] != (EnvVar{Name: envARCADIA_ROOT_DISTBUILD, Value: strS.any()}) {
...
func TestGen_UseArcadiaLibm_NoSelfPeer(t *testing.T) {
	...
	got := (&EmitContext{ctx: &GenCtx{}, instance: mi, d: &ModuleData{useArcadiaLibm: true}}).defaultProgramPeerdirsForWithState(false)
