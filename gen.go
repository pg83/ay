...
func runGenIntoWithResources(srcRoot, targetDir string, hostP, targetP *Platform, emitter Emitter, onWarn func(Warn), resources *resourceFetchPlan, testMode bool, materializeResourceFetches bool) NodeRef {
	...
	// VFS ids are the global intern-table indices, so the two scanners share
	// that id space with no per-run interner.
...
func genModule(ctx *genCtx, instance ModuleInstance) *moduleEmitResult {
	...
	// No regular member-input aggregation: an AR/LD node's inputs are its
	// objects/archives + its own scripts (emitted directly below); the
	// members' source/header closure and the codegen wrapper scripts that
	...
	// closure into AR/LD inputs; that closure isn't carried anymore, so the
	// walk is gone — saving a full closure scan per SRCS header.)
