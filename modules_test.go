...
func TestCopyFileInputVFS_ResolvesSourceRootPaths(t *testing.T) {
	...
	if got := copyFileInputVFS(fs, dirKey("mod").source(), "local.txt").string(); got != "$(S)/mod/local.txt" {
		t.Fatalf("local copy input = %q, want $(S)/mod/local.txt", got)
	}

	if got := copyFileInputVFS(fs, dirKey("mod").source(), "shared/generated.txt").string(); got != "$(S)/shared/generated.txt" {
		t.Fatalf("root copy input = %q, want $(S)/shared/generated.txt", got)
	}

	if got := copyFileInputVFS(fs, dirKey("pkg/sub").source(), "pkg/sub/codecs.h").string(); got != "$(S)/pkg/sub/codecs.h" {
...
func TestApplyDeclareInDirsAcceptsExpandedSourceRoot(t *testing.T) {
	fs := newMemFS(map[string]string{
		"first/options.json":  "{}\n",
		"second/options.json": "{}\n",
		"second/other.json":   "{}\n",
	})
	env := buildIfEnv(ModuleInstance{Path: source("mod"), Platform: testTargetP})

	applyDeclareInDirs(fs, "mod", UnknownStmt{Args: anysOf(
		"OPTIONS",
		"options.json",
		"DIRS",
		"first",
	...
	if got, _ := env.lookup("OPTIONS_FILES"); got != "$(S)/first/options.json $(S)/second/options.json" {
		t.Fatalf("OPTIONS_FILES = %q, want files from both source-root directories", got)
	}

	if got, _ := env.lookup("OPTIONS_SRCDIR"); got != "$(S)" {
		t.Fatalf("OPTIONS_SRCDIR = %q, want the expanded source-root marker preserved", got)
	}
}

func TestCollectModuleDeclareInDirsFeedsFollowingRunProgram(t *testing.T) {
	const src = `LIBRARY()
SET(DIRECTORIES
    first
    second
)
DECLARE_IN_DIRS(OPTIONS options.json DIRS $DIRECTORIES SRCDIR ${ARCADIA_ROOT})
RUN_PROGRAM(
...
`
	...
	env := buildIfEnv(ModuleInstance{Path: source("mod"), Kind: KindLib, Platform: testTargetP})
	d := collectModule(newIncludeParserManagerFS(fs, newSharedParseCache()), ModuleInstance{Path: source("mod"), Kind: KindLib},
		mf.Stmts, env, noWarn)
	...
	want := []string{"$(S)/first/options.json", "$(S)/second/options.json"}

	if got := anyStrs(d.runPrograms[0].INFiles); !equalStrings(got, want) {
		filesValue, _ := env.lookup("OPTIONS_FILES")
		dirsValue, _ := env.lookup("DIRECTORIES")
		t.Fatalf("RUN_PROGRAM IN = %v, want %v; OPTIONS_FILES=%q DIRECTORIES=%q", got, want, filesValue, dirsValue)
	}
}
...
func testToolchain() ModuleToolchain {
	return resolveModuleToolchain(&GenCtx{tcMemo: map[ToolchainKey]ModuleToolchain{}}, []ResourceDecl{
