...
func TestSimdVariantForAVX2(t *testing.T) {
	...
	if v.Suffix.string() != "avx2" {
		t.Fatalf("Suffix = %q, want avx2", v.Suffix)
	}

	if !equalStrings(anyStrs(v.CFlags), []string{"-mavx2", "-mfma", "-mbmi", "-mbmi2"}) {
...
func TestSimdVariantForAVX512(t *testing.T) {
	...
	if v.Suffix.string() != "avx512" {
		t.Fatalf("Suffix = %q, want avx512", v.Suffix)
	}

	if !equalStrings(anyStrs(v.CFlags), []string{"-mavx512f", "-mavx512cd", "-mavx512bw", "-mavx512dq", "-mavx512vl"}) {
		t.Fatalf("CFlags = %v", v.CFlags)
	}
}
...
func TestSimdVariantForAMX(t *testing.T) {
	...
	if v.Suffix.string() != "amx" {
		t.Fatalf("Suffix = %q, want amx", v.Suffix)
	}

	if !equalStrings(anyStrs(v.CFlags), []string{
		"-mamx-tile", "-mamx-int8",
